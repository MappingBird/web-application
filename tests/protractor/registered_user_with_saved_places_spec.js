// registered_user_with_saved_places_spec.js
describe('Registered user with saved places', function() {
    ...
    it('should be able to sign up', function() {
        ...
        var email = 'mhpalmer' + getRandomInt(0,100) + getRandomInt(0,100) + '@gmail.com',
            password = 'asdfasdf';

        browser.get(baseUrl + '/static/signup.html');
        browser.sleep(5000);
        element(by.model('email')).sendKeys(email);
        element(by.model('password')).sendKeys(password);
        element(by.css('.action-button input')).click();
        browser.sleep(1000);
        expect(element(by.id('container-account-created')).isDisplayed()).toBe(true);
    ...
    it('should display the registered user menu', function() {

        expect(element(by.id('registered-user-menu')).isDisplayed()).toBe(true);

    });

    it('should hide the unregistered user menu', function() {

        expect(element(by.id('unregistered-user-menu')).isDisplayed()).toBe(false);
    ...
    it('should display saved places when click on "View Map" menu items', function() {
        ...
        browser.get(baseUrl + '/static/index.html?stay=1');
        browser.sleep(5000);
        element(by.css('#registered-user-menu .dropdown-toggle')).click();
        element(by.css('#registered-user-menu a[href="/static/app.html#/"]')).click();
        browser.sleep(5000);
        expect(element(by.css('.save-panel')).isDisplayed()).toBe(true);
